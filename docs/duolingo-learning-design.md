...
- 统计页连胜卡片旁显示"盾牌 SVG 图标 + 保护券 ×N"（使用 `ShieldIcon` 组件，禁止 emoji）。
...
**体验**：
- 首页"今日学习提醒"卡片下方，新增一行小进度：`[TargetIcon] 2/3 · 复习 10 ✓ · 答对 15 (12) · 查词 ✓`（使用 `TargetIcon` SVG 组件，禁止 emoji）
- 复习页顶部进度条旁，并列显示任务进度。
...
**徽章设计**（8 类，每类 3-4 级，共 28 个）：

| 类别 | 徽章示例 | 触发条件 |
|------|----------|----------|
| **坚持 streak** | streak-7 / streak-30 / streak-100 / streak-365 | Streak 里程碑 |
| **积累 vocab** | vocab-100 / vocab-500 / vocab-2000 / vocab-5000 | card: 计数 |
| **精度 accuracy** | accuracy-7 / accuracy-30 / accuracy-perfect | StudyLog 派生 |
| **探索 exploration** | explore-10 / explore-50 / explore-200 / explore-1000 | 搜索历史派生 |
| **突破 breakthrough** | session-50 / session-100 / early-bird / night-owl | 单次会话 / 时段彩蛋 |
| **收藏家 book** | book-quarter / book-half / book-complete | 词书完成度 |
| **精通 mastery** | mastered-50 / mastered-200 / mastered-1000 | 卡片掌握数（V4/mastered） |
| **隐藏 secret** | secret-comeback / secret-shield-saver / secret-quiet-day | 彩蛋（??? 显示） |

**图标规范**：所有徽章图标使用 `BadgeIcon` 组件（切面宝石勋章，产品视觉签名），`BadgeRule.icon` 字段存 icon key 字符串（如 `"flame"`、`"target"`、`"keyhole"`），由组件按 key 渲染对应 SVG glyph。**禁止使用 emoji 作为 UI 图标**（见 AGENTS.md 图标规范）。

**体验**：
- 统计页新增"成就"区块，未解锁的徽章灰显+解锁条件，已解锁的亮起+解锁日期。
- 解锁瞬间顶部 toast："解锁「积累 · 500 词」"（配 BadgeIcon 切面宝石勋章，非 emoji）。
- 隐藏徽章解锁前显示 ??? + keyhole 图标，仅显示类别不显示规则。
- 不做炫耀分享——单人产品，自我满足足够。

**为什么这样设计**：徽章是多邻国 +30% 完成率的功臣。但多邻国的徽章有几十个，膨胀了。WordFlow 做 28 个、8 大类，每个都对应真实的学习行为，没有"分享到朋友圈"这种虚荣徽章。4 级稀有度（铜/银/金/钻石），钻石级额外奖励 1 张连胜保护券。**少即是多，但要有层次。**
