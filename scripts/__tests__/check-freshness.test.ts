...
describe("checkFreshness", () => {
  test("dict 目录不存在 → errors 含 'dict 目录不存在'", () => {
    const result = checkFreshness(TMP);
    expect(result.total).toBe(0);
    expect(result.errors).toContain("dict 目录不存在");
  });
  ...
  test("空切片 → empty 计数 + errors", () => {
    mkdirSync(join(TMP, "dict", "a"), { recursive: true });
    writeFileSync(join(TMP, "dict", "a", "empty.json"), "", "utf8");
    const result = checkFreshness(TMP);
  ...
  test("过期切片（mtime 超过阈值）→ stale 计数", () => {
    mkdirSync(join(TMP, "dict", "a"), { recursive: true });
    writeFileSync(
      join(TMP, "dict", "a", "old.json"),
      '[{"word":"old"}]',
      "utf8"
    );
    // 把 mtime 设为 400 天前
    const old = new Date(Date.now() - 400 * 24 * 60 * 60 * 1000);
    utimesSync(join(TMP, "dict", "a", "old.json"), old, old);
  ...
  test("maxAgeDays 足够大 → 不报过期", () => {
    mkdirSync(join(TMP, "dict", "a"), { recursive: true });
    writeFileSync(
      join(TMP, "dict", "a", "ok.json"),
      '[{"word":"ok"}]',
      "utf8"
    );
    const old = new Date(Date.now() - 100 * 24 * 60 * 60 * 1000);
    utimesSync(join(TMP, "dict", "a", "ok.json"), old, old);
    const result = checkFreshness(TMP, 365);
